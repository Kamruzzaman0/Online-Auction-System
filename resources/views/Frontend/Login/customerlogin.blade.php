<div class="tile">
  ...
  <div class="tile-header">
    <h2 style="color: white; opacity: .75; font-size: 2rem; display: flex; justify-content: center; align-items: center; "></h2>
  </div>
  
  <div class="tile-body">
    <form id="form"  action="{{route('login1')}}" method="post">
        @csrf
      <label class="form-input">
        <i class="material-icons">person</i>
        <input type="text" name="email" autofocus="true" required />
      ...
      <button id="submit" role="button" class="btn btn-irenic float-right" tabindex="0" >
          <span>Login</span>
        </div>
       </button>
        <div class="submit-container clearfix" style="margin-top: 2rem;">          
        <p class="message">Not registered? </p>
        <p><a href="{{route('customer_registration')}}">Create an account</a></p>
        </div>
      </div>
    </form>
  </div>
...
.tile .tile-header {
  background-image: url("http://www.fpsaus.com.au/images/customer-login-icon.gif");
